...
class SSRNet(nn.Module):
    def __init__(self, stage_num=[3, 3, 3], image_size=64,  # 暂时只能用64大小的
                 class_range=70, lambda_index=1., lambda_delta=1.):
    ...
    def forward(self, image_):
        ...
        # print(embedding_stream1_stage1_before_PB.size(), "11")
        # print(embedding_stream2_stage1_before_PB.size(), "21")
        stage1_delta_k = self.stage1_delta_k(
            torch.mul(embedding_stream1_stage1_before_PB, embedding_stream2_stage1_before_PB))
        # print(embedding_stream1_stage2_before_PB.size(), "12")
        # print(embedding_stream2_stage2_before_PB.size(), "22")
...
def demo_test():
    ...
    net = net.cuda('cuda')
    net.eval()
    x = torch.randn(1, 3, 128, 128).cuda('cuda')
    test_numbers_ = 1000
    a_time = time.time()
